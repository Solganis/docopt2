...
concrete invocations your usage message accepts.

It answers one narrow question, *what does my usage actually accept?*, and hands you the answer as data you
can diff, replay, or fuzz against. It never touches your program's real input, and it is not a test generator
in the "assert the right answer" sense.

The examples are walked out of the same pattern tree [`docopt`](../reference/docopt.md) matches against, so
they cannot drift from what it parses. Narrow or widen the usage, and the generated examples move with it.

!!! note "Structural, not semantic"
    Positionals and option values are filled with placeholders (`v1`, `v2`, ...), not realistic domain values.

    The generator samples the *structure* of accepted invocations, which commands, options and positionals may
    combine, never whether a `<port>` is a valid port. It exercises the shape of your CLI surface, and your
    own validation stays a separate concern.
...
The sampler covers every construct:

- alternation picks a branch (`mine set` against the `ship` lines)
- a `...` repetition emits one or more values (`new v4 v5`)
- an optional group is taken or skipped (`[--speed=<kn>]`)
- each positional or valued option gets a fresh placeholder

`seed` makes the output reproducible. The same seed yields the same list, so you can commit it as a golden
file.

Duplicates are dropped, so a grammar with few accepted shapes returns fewer than `count`:
...
`valid=False` returns argvs the usage *rejects*: a valid shape with an undefined option appended.

That gives you the reject-set to check your program's error handling against, the same way the valid set
...
`docopt2 examples <source>` prints one example invocation per line.

The usage is read from a `.py` module docstring (without importing it), a text file of raw usage, or `-` for
standard input:
...
Unlike a fixed list from `generate_examples`, the strategy keeps exploring new shapes across runs, and shrinks
a failure to the smallest argv that still triggers it.

A malformed usage raises [`DocoptLanguageError`](../reference/exceptions.md) at once, before any draw.
