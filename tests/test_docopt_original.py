# Ported verbatim from the original docopt test suite (MIT): the compatibility baseline.
# Do not rewrite the assertions; only the import is retargeted to docopt2.
...
def test_pattern_flat():
    ...
    assert Required(Optional(OptionsShortcut()), Optional(Option("-a", None))).flat(OptionsShortcut) == [
        OptionsShortcut()
    ]
...
def test_parse_pattern():
    ...
    assert parse_pattern("[options] A", options=o) == Required(Optional(OptionsShortcut()), Argument("A"))
    assert parse_pattern("-v [options]", options=o) == Required(Option("-v", "--verbose"), Optional(OptionsShortcut()))
...
def test_option_match():
    assert Option("-a").match([Option("-a", value=True)]) == (True, [], [Option("-a", value=True)])
...
def test_command_match():
    ...
    assert Either(Command("add", False), Command("rm", False)).match([Argument(None, "rm")]) == (
        True,
        [],
        [Command("rm", True)],
    )
...
def test_optional_match():
    ...
    assert Optional(Option("-a")).match([Option("-x")]) == (True, [Option("-x")], [])
    assert Optional(Option("-a"), Option("-b")).match([Option("-a")]) == (True, [], [Option("-a")])
    assert Optional(Option("-a"), Option("-b")).match([Option("-b")]) == (True, [], [Option("-b")])
    assert Optional(Option("-a"), Option("-b")).match([Option("-x")]) == (True, [Option("-x")], [])
    assert Optional(Argument("N")).match([Argument(None, 9)]) == (True, [], [Argument("N", 9)])
    assert Optional(Option("-a"), Option("-b")).match([Option("-b"), Option("-x"), Option("-a")]) == (
        True,
        [Option("-x")],
        [Option("-a"), Option("-b")],
    )
...
def test_either_match():
    ...
    assert Either(Option("-a"), Option("-b"), Option("-c")).match([Option("-x"), Option("-b")]) == (
        True,
        [Option("-x")],
        [Option("-b")],
    )
...
def test_one_or_more_match():
    ...
    assert OneOrMore(Argument("N")).match([Argument(None, 9), Option("-x"), Argument(None, 8)]) == (
        True,
        [Option("-x")],
        [Argument("N", 9), Argument("N", 8)],
    )
    assert OneOrMore(Option("-a")).match([Option("-a"), Argument(None, 8), Option("-a")]) == (
        True,
        [Argument(None, 8)],
        [Option("-a"), Option("-a")],
    )
    ...
    assert OneOrMore(Optional(Argument("N"))).match([Argument(None, 9)]) == (True, [], [Argument("N", 9)])


def test_list_argument_match():
    assert Required(Argument("N"), Argument("N")).fix().match([Argument(None, "1"), Argument(None, "2")]) == (
        True,
        [],
        [Argument("N", ["1", "2"])],
    )
    assert OneOrMore(Argument("N")).fix().match([Argument(None, "1"), Argument(None, "2"), Argument(None, "3")]) == (
        True,
        [],
        [Argument("N", ["1", "2", "3"])],
    )
    ...
    assert Required(Argument("N"), Required(Argument("N"))).fix().match([Argument(None, "1"), Argument(None, "2")]) == (
        True,
        [],
        [Argument("N", ["1", "2"])],
    )
...
def test_pattern_either():
    ...
    assert transform(Optional(Option("-a"), Either(Option("-b"), Option("-c")))) == Either(
        Required(Option("-b"), Option("-a")), Required(Option("-c"), Option("-a"))
    )
    assert transform(Either(Option("-x"), Either(Option("-y"), Option("-z")))) == Either(
        Required(Option("-x")), Required(Option("-y")), Required(Option("-z"))
    )
...
def test_set():
    ...
    assert {Argument("N"), Argument("N")} == {Argument("N")}
...
def test_docopt():
    ...
    a = docopt(doc, "-v")
    assert a == {"-v": True, "-q": False, "-r": False, "--help": False, "FILE": None, "INPUT": None, "OUTPUT": None}
...
def test_options_first():
    assert docopt("usage: prog [--opt] [<args>...]", "--opt this that") == {"--opt": True, "<args>": ["this", "that"]}
    assert docopt("usage: prog [--opt] [<args>...]", "this that --opt") == {"--opt": True, "<args>": ["this", "that"]}
    assert docopt("usage: prog [--opt] [<args>...]", "this that --opt", options_first=True) == {
        "--opt": False,
        "<args>": ["this", "that", "--opt"],
    }
