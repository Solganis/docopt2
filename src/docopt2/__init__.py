...
from docopt2._completion import complete, generate_completion, reply_to_completion_request
...
# __all__ also re-exports the parser primitives (Tokens, formal_usage, parse_*, transform) for
# drop-in compatibility with the original docopt module, whose users import them directly.
__all__ = [
    "Argument",
    "Arguments",
    "BranchPattern",
...
def _extras(default_help: bool, version: object, options: list[Pattern], doc: str) -> None:
    ...
    if not any(leaf.name == "--" for leaf in pattern.flat(Command)):
        for index, leaf in enumerate(argv_patterns):
            if type(leaf) is Argument and leaf.value == "--":
                ...
                break
    _extras(show_help, version, argv_patterns, doc)
...
class Dispatch:
    ...
    def run(self, argv: list[str] | tuple[str, ...] | str | None = None, **options: Any) -> Any:
        ...
        return handler(arguments if schema is None else bind_schema(arguments, schema))
